# 当前架构说明

更新时间：2026-09-12

## 1. 架构概览

当前项目由 `app` 主应用模块和 `benchmark` 性能测试模块组成，主应用内部按 `feature`、`data`、`common`、`automation` 分层组织。

## 2. 代码分层

### 2.1 feature

- `feature.home`
  - 桌面主页
  - 桌面卡片、拖拽排序、低性能模式刷新逻辑
- `feature.appmanage`
  - 应用管理页
  - 应用列表展示与勾选逻辑
- `feature.phone`
  - 电话联系人页
  - `PhoneContactAdapter`
  - 联系人列表/宫格布局、头像编辑和系统拨号入口
- `feature.videocall`
  - 视频联系人页
  - `VideoCallContactAdapter`
  - `VideoCallCoordinator`
  - `VideoContactDialogController`
- `feature.settings`
  - 家属设置首页、详情页、统一对话框、权限与系统设置入口
  - 低性能模式、主题、首页布局和版本更新检查
- `feature.incoming`
  - 系统电话来电页、默认电话角色与 `InCallService`
  - 来电事件合并、联系人匹配、厂商策略和接听结果诊断
- `feature.weather`
  - 应用内天气详情、城市管理和城市卡片展示

### 2.2 data

- `data.home`
  - `LauncherPreferences`
  - `LauncherAppRepository`
  - `HomeAppOrderPolicy`
- `data.contact`
  - `Contact`、`ContactManager`、`ContactSqliteStore`、`ContactStorage`、`ContactAvatarStore`
  - 电话联系人和视频联系人共享 SQLite 数据库，通过 `group_key` 区分 `phone` 与 `wechat`
  - 所有写操作为 `suspend fun`，持久化通过 `Dispatchers.IO.limitedParallelism(1)` 串行化
- `data.weather`
  - `WeatherRepository`、`WeatherDiskCache`、`WeatherPreferences`
  - Open-Meteo 数据源、天气解析、当前位置解析和缓存回退
- `data.settings`
  - 家属设置持久化与页面状态

### 2.3 common

- `common.media`
  - 应用图标与 URI 缩略图加载（`MediaThumbnailLoader`）
- `common.service`
  - `TTSService`
- `common.ui`
  - `PageStateView`
  - `FloatingStatusView`
  - 头像裁剪与编辑控制器
- `common.lobster`
  - 设备身份、构建身份、会话事件序列、日志脱敏、离线队列和结构化诊断上报
  - 主线程阻塞采样会区分真实阻塞与采样时已恢复，避免把空闲栈计为错误
- `common.perf`
  - Trace 与性能指标收集
- `common.util`
  - `PermissionRequestHandler`
  - `PermissionUtil`
  - `NetworkUtil`
  - `AccessibilityServiceMatcher`
  - `DebugLog`
  - `VersionFormatter`
### 2.4 automation

- `automation.wechat`
  - 实验性微信自动化能力
  - 包含状态模型、状态检测、超时管理、无障碍服务和辅助工具

### 2.5 testing

- `app/src/test`
  - 单元测试（421 tests, 0 failed）与 Robolectric UI 冒烟
- `app/src/androidTest`
  - 设备级仪器测试
- `benchmark`
  - Baseline Profile 与 Macrobenchmark

## 3. 数据与存储

- `launcher_prefs`
  - 桌面已选择应用
  - 桌面应用顺序
  - 低性能模式开关
- `launcher_contacts.db`
  - 共享联系人 SQLite 数据库
  - `group_key = "phone"` 存储电话联系人
  - `group_key = "wechat"` 存储视频联系人
- `contacts` / `phone_contacts` / `wechat_contacts`
  - 旧版 SharedPreferences JSON 数据源，仅作为历史迁移兼容边界
- `ContactsContract`
  - 系统通讯录，仅在导入时只读访问

## 4. 外部系统依赖

- Android Launcher 能力
- Contacts Provider
- 系统照片选择器
- 电话拨号权限、默认电话角色与 Telecom API
- Open-Meteo 与前台粗略定位
- 系统设置页跳转
- 无障碍服务（微信自动化）
- 悬浮窗权限（微信自动化）
- Lobster 日志、诊断与性能指标上报

## 5. 当前架构特点

- 无账号、无业务后端，核心数据本地优先
- 可选 Lobster 日志与性能指标上报，用于线上诊断和自有网站分析
- 结构化诊断使用 v5 契约，通过构建 SHA 和会话事件序列把线上样本对应到具体源码与故障前操作链；详见 `docs/specs/structured-diagnostics-v5.md`
- 已有轻量 SQLite 联系人数据层，设置和桌面应用配置仍依赖 SharedPreferences
- 没有依赖注入框架
- 主业务仍集中在单个 `app` 模块
- 联系人和天气数据操作已迁移至后台线程，主线程不做持久化 IO
- 空状态视图和权限流程已经形成公共抽象
- 微信视频自动化已补齐服务中断、请求超时、失败回前台和步骤级诊断，但仍依赖具体微信 UI 与无障碍能力
- 自动化能力与主业务已做包级隔离，但仍处在同一应用包内
- 测试层已经拆分为单元测试、Robolectric 与设备级仪器测试三层
- Android 资源遵循固定 `res` 目录，通过功能前缀和拆分后的 `values/*.xml` 分类

## 6. 当前仍需关注的点

- `automation.wechat` 仍需要持续做多机型、多微信版本和弱网场景验证
- 系统电话自动接听（`feature.incoming`）仍需真实 SIM 和更多 ROM 验证
- 首页视觉仍在 v2.0.0 发布前收尾
- 低性能模式、权限拒绝、系统设置跳转和跨页面链路仍缺更完整的设备级回归
- 设置、桌面应用选择和部分状态仍依赖 SharedPreferences，没有更强的状态恢复层

## 7. 当前架构边界

- 主页、应用管理、电话、设置、视频联系人属于当前主业务
- 微信自动化属于实验性能力
- 微信视频来电自动接听、通知监听、远程协助不在当前交付范围内
- 系统电话自动接听的软件链路已完成，设备兼容性待补齐
