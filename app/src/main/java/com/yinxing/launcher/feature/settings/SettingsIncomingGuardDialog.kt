...
internal fun SettingsActivity.showIncomingGuardDialog(): android.app.Dialog {
    ...
    listOf(PermissionGroup.Call, PermissionGroup.KeepAlive).forEach { group ->
        val section = addDialogSection(dialog, getString(group.titleRes))
        readiness.items
            .filter { it.item.toPermissionEntry() in group.entries }
            .forEach { itemState ->
